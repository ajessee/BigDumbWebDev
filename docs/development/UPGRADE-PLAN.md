...
- [ ] **TODO, found 2026-09-14:** guard `User#guess_city`/`guess_region`/`guess_country`/`guess_address` against a nil `Geocoder.search` result (or nil `ip_address` directly) — see the corrected Known issues entry above. `Geocoder` is not dead code and this crashes real profile views today, independent of the upgrade.
