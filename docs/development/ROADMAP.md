...
- [ ] **New, found during Phase 1's rollout, not yet triaged.** Pushing to GitHub after the `master`→`main` rename briefly showed alarming Dependabot counts (476, then 402, then 133) that turned out to be GitHub still re-indexing against the new dependency tree, not real findings — confirmed via `gh api repos/ajessee/BigDumbWebDev/dependabot/alerts` directly rather than trusting the push-output number. **The real, settled count is 8 open alerts: 6 medium, 2 low, 0 critical/high.** Seven of the eight are `trix` (npm) — mostly stored/reflected XSS via paste, drag-and-drop, and attachment attributes — plus one unrelated `uuid` buffer-bounds issue. Worth a real look specifically because Trix is central to this app's rich-text feature (posts/comments) and already has a custom sanitizer allow-list (`config/initializers/action_text.rb`) that a Trix version bump would need to re-verify, same as the horizontal-rule regression found during the Shakapacker upgrade.
