...
**A Rails 6.1 → 8.1 upgrade is in progress.** Before making any non-trivial change, read [docs/development/UPGRADE-PLAN.md](docs/development/UPGRADE-PLAN.md) (a source-level audit of exactly what's outdated, broken, or risky in this codebase, updated as work progresses) and [docs/development/UPGRADE-LEARNINGS.md](docs/development/UPGRADE-LEARNINGS.md) (lessons carried over from the sibling RDJesseeBlog app's completed identical upgrade). Both docs contain specific, already-confirmed findings (not speculation) about this exact codebase. For anything touching DNS, domain registration, or email deliverability, read [docs/development/DOMAIN-AND-EMAIL.md](docs/development/DOMAIN-AND-EMAIL.md) first — the registrar/DNS host, existing email routing, and existing SendGrid domain-auth records are not what you'd guess from the Heroku add-on list alone.
...
- Dev server: `bin/rails server` — **also requires `bin/shakapacker-dev-server` running in a separate process** for JS/CSS to recompile (standard Shakapacker dual-process setup)
